...
pub enum State {
    ...
    EndTagOpen,
    /// https://html.spec.whatwg.org/multipage/parsing.html#end-tag-name-state
    TagName,
    ...
    /// https://html.spec.whatwg.org/multipage/parsing.html#attribute-value-(quoted)-state
    AttributeValueUnquoted,
    /// https://html.spec.whatwg.org/multipage/parsing.html#after-attribute-value-(quoted)-state
    AfterAttrbuteValueQuoted,
    ...
    /// https://html.spec.whatwg.org/multipage/parsing.html#script-data-less-end-tag-open-state
    ScriptDataEndTagOpen,
    /// https://html.spec.whatwg.org/multipage/parsing.html#script-data-less-end-tag-name-state
    ...
    fn append_tag_name(&mut self, c: char) {
        ...
        if let Some(t) = self.latest_token.as_mut() {
            match t {
                ...
                _ => panic!("`latest_token` shoud be either StartTag or EndTag"),
            }
        }
    }
        ...
        loop {
            ...
            match self.state {
                ...
                State::AttributeValueDoubleQuoted => {
                    if c == '"' {
                        self.state = State::AfterAttrbuteValueQuoted;
                        continue;
                    }
                ...
                State::AfterAttrbuteValueQuoted => {
                    if c == ' ' {
                        self.state = State::BeforeAttributeName;
                        continue;
                    }
                ...
                State::ScriptDataEndTagOpen => {
                    ...
                    // この実装ではnextメソッドからは1つのトークンしか返せないため、
                    // "<"のトークンのみを返す
                    return Some(HtmlToken::Char('<'));
                }
                ...
                State::TemporaryBuffer => {
                    ...
                    let c = self
                        ...
                        .chars()
                        .nth(0)
                        .expect("self.buf have at least 1 char");
                    self.buf.remove(0);
                    return Some(HtmlToken::Char(c));
                }
            }

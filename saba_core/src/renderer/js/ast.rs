...
impl JsParser {
        ...
        match t {
            Token::Punctuator(c) => match c {
                '+' | '-' => {
                    // '+'または'-'の記号を消費する
                    assert!(self.t.next().is_some());
                    Node::new_additive_expression(c, left, self.additive_expression())
                }
                _ => left,
            },
            _ => left,
    ...
    fn primary_expression(&mut self) -> Option<Rc<Node>> {
        ...
        match t {
            Token::Identifier(value) => Node::new_identifer(value),
    ...
    fn identifier(&mut self) -> Option<Rc<Node>> {
        ...
        match t {
            Token::Identifier(name) => Node::new_identifer(name),
            _ => None,
        }
    }
    ...
    fn parameter_list(&mut self) -> Vec<Option<Rc<Node>>> {
        ...
        match self.t.next() {
            Some(t) => match t {
                ...
                _ => unimplemented!("function shoud have `(` but got {:?}", t),
            },
            None => unimplemented!("function shoud have `(` but got None"),
        }
    ...
    fn function_body(&mut self) -> Option<Rc<Node>> {
        ...
        match self.t.next() {
            Some(t) => match t {
                Token::Punctuator(c) => assert!(c == '{'),
                _ => unimplemented!("function shoud have open curly blacket but got {:?}", t),
            },
            None => unimplemented!("function shoud have open curly blacket but got None"),
        }
